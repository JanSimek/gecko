...
| [`fills/quilt_desert.luau`](fills/quilt_desert.luau) | **Quilting as a point-and-click fill**: select an area in the editor, Edit ▸ Fill Selection, pick *Quilted Desert* — the selection's floor is synthesized from the reference map with live ghost preview and a one-Ctrl-Z apply. Headless (`generate`) it quilts the whole elevation; `--arg ref=` / `--arg refElevation=` override the reference. Best for organic, unstructured ground; authored compositions (a cliff, a building) belong in a **stamp** (`extract_pattern` → `api:placeStamp`), not a quilt. |
...
| `export_protos` | **Every** item and critter proto the game can load — each entry of `proto/items/items.lst` and `proto/critters/critters.lst`, not only what some map places, so a weapon that only turns up in a script-stocked shop is included — with the full stats its `.pro` stores, decoded the way fallout2-ce reads it. Common: `pid`, `kind`, `file`, `name`, `description`, `fid`, `flags`, `extendedFlags`, `sid`, `script`. Items add `itemType`, `attackModes` (`primary`/`secondary` from the extended-flags nibbles, each `{index,type,animation}` per the engine's `_attack_subtype`/`_attack_anim` tables), `material`, `size`, `weight`, `cost`, `inventoryFid`, `soundId`, and one object named after the type: `weapon` (damage, damage type, ranges, AP costs, min ST, perk, burst, caliber, ammo…), `ammo` (`acModifier`, `drModifier`, `damageMultiplier`, `damageDivisor`), `armor` (`ac`, `dr`/`dt` keyed by damage type, `perk`, male/female FIDs), `drug`, `container`, `misc`, `key`. Critters add `headFid`, `aiPacket`, `team`, `critterFlags`, `base` and `bonus` stat blocks, raw `skills`, `bodyType`, `xp`, `killType`, `damageType`. Raw values — no engine formulas; ids the game names (damage type, caliber, material, body type from `proto.msg`, perk from `perk.msg`, stat from `stat.msg`) come as `{id,name}`. `unreadable` lists `.lst` entries that failed to load. Optional `kind` (`item`/`critter`) and `itemType` (`armor`, `container`, `drug`, `weapon`, `ammo`, `misc`, `key`). |
...
- `proto export [--kind item|critter] [--item-type <type>]` (also the MCP `export_protos` tool)
