...
| `analyze` | The full `analyze --json` report (omit `maps` for all, or scope it): per-map and aggregate floor tiles, objects (each with `number` = the `api:proto` id, plus the `flat` palette-curation flag), `adjacency` — the floor-tile borders (which tile sits next to which different tile), i.e. the transitions to curate for seamless terrain — and per-map `clusters`: nearby objects grouped into structures (tents, buildings), each with a `centerHex`, bounding box and member PIDs, so an agent can locate one and feed its anchor/PIDs to `extract_pattern`; and per-map `critters`: each critter with its `hex`, `team` (group_id), effective `aiPacket`, and the AI resolved via `ai.txt` (`ai`: name, aggression, disposition, run-away / best-weapon / distance), so an agent can read who is on the map and how they fight. |
