...
- **B2 — Persistent VM + lifecycle + manifest (no UI registration, read-only `api`).** Landed so far: `PluginVm` (allocator cap, watchdog, bounded console, `pcall` isolation, auto-disable on fault with GC-on-fault for deterministic OOM recovery), ~~`MapScriptApi::retarget` with the full null-safe audit~~ (done), and the **read-only `api`** (the `GECK_SCRIPT_API` READ/WRITE split: a read-only host binds only the READ half, so mutators are absent from the class — no permission check to bypass; `PluginVm` defaults read-only via `Config::allowMapWrite=false`). ~~Remaining: `PluginManifest` parse, `PluginManager` discovery/enable/disable, basic Plugin Manager dialog~~ (done): `PluginManifest` (validated `plugin.json`, no silent fallbacks), `PluginManager` (scan user + bundled dirs, dedupe by id with user shadowing, one resident read-only `PluginVm`+`MapScriptApi` per enabled plugin, `setEditorBinding`/`clearEditorBinding` retarget-on-map-open / detach-on-close, fault-marked rows), a `PluginManagerDialog` (list/enable/disable/rescan/console), `MapScriptApi` gains a detached default constructor for the resident owner, and a bundled `hello` example plugin. **The plugin MVP is complete.** Next: B3 (UI/tool registration + `map.write` permission model).
