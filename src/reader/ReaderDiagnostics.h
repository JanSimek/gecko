...
struct ParseStatistics {
    ...
    double throughputMBps() const {
        if (parseTime.count() == 0) return 0.0;
        return (static_cast<double>(fileSize) / (1024.0 * 1024.0)) / 
               (static_cast<double>(parseTime.count()) / 1000000.0);
    }
...
public:
    ...
    public:
        ParseTimer() : startTime(std::chrono::high_resolution_clock::now()) {}
    ...
    static void logParseComplete(const std::filesystem::path& filePath, 
        ...
        size_t fileSize = std::filesystem::file_size(filePath);
        double throughput = (static_cast<double>(fileSize) / (1024.0 * 1024.0)) / 
                           (static_cast<double>(parseTime.count()) / 1000000.0);
