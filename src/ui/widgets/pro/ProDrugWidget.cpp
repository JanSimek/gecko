...
void ProDrugWidget::setupUI() {
    ...
    timingGroup->setStyleSheet("QGroupBox { font-weight: bold; }");
...
void ProDrugWidget::loadFromPro(const std::shared_ptr<Pro>& pro) {
    ...
    for (int i = 0; i < NUM_DRUG_STATS; ++i) {
        ...
        if (_drugStatAmountEdits[i]) _drugStatAmountEdits[i]->setValue(static_cast<int>(amounts[i]));
        if (_drugFirstStatAmountEdits[i]) _drugFirstStatAmountEdits[i]->setValue(static_cast<int>(firstAmounts[i]));
        if (_drugSecondStatAmountEdits[i]) _drugSecondStatAmountEdits[i]->setValue(static_cast<int>(secondAmounts[i]));
    }
    
    if (_drugFirstDelayEdit) _drugFirstDelayEdit->setValue(static_cast<int>(_drugData.duration1));
    if (_drugSecondDelayEdit) _drugSecondDelayEdit->setValue(static_cast<int>(_drugData.duration2));
    if (_drugAddictionChanceEdit) _drugAddictionChanceEdit->setValue(static_cast<int>(_drugData.addictionRate));
    if (_drugAddictionPerkCombo) _drugAddictionPerkCombo->setCurrentIndex(static_cast<int>(_drugData.addictionEffect));
    if (_drugAddictionDelayEdit) _drugAddictionDelayEdit->setValue(static_cast<int>(_drugData.addictionOnset));
}
...
void ProDrugWidget::saveToPro(std::shared_ptr<Pro>& pro) {
    ...
    for (int i = 0; i < NUM_DRUG_STATS; ++i) {
        ...
        if (_drugStatAmountEdits[i]) amounts[i] = static_cast<int32_t>(_drugStatAmountEdits[i]->value());
        if (_drugFirstStatAmountEdits[i]) firstAmounts[i] = static_cast<int32_t>(_drugFirstStatAmountEdits[i]->value());
        if (_drugSecondStatAmountEdits[i]) secondAmounts[i] = static_cast<int32_t>(_drugSecondStatAmountEdits[i]->value());
    }
    
    // Map arrays back to data structure
    _drugData.stat0 = stats[0]; _drugData.stat1 = stats[1]; _drugData.stat2 = stats[2];
    _drugData.amount0 = amounts[0]; _drugData.amount1 = amounts[1]; _drugData.amount2 = amounts[2];
    _drugData.amount0_1 = firstAmounts[0]; _drugData.amount1_1 = firstAmounts[1]; _drugData.amount2_1 = firstAmounts[2];
    _drugData.amount0_2 = secondAmounts[0]; _drugData.amount1_2 = secondAmounts[1]; _drugData.amount2_2 = secondAmounts[2];
    
    if (_drugFirstDelayEdit) _drugData.duration1 = static_cast<uint32_t>(_drugFirstDelayEdit->value());
    if (_drugSecondDelayEdit) _drugData.duration2 = static_cast<uint32_t>(_drugSecondDelayEdit->value());
    if (_drugAddictionChanceEdit) _drugData.addictionRate = static_cast<uint32_t>(_drugAddictionChanceEdit->value());
    if (_drugAddictionPerkCombo) _drugData.addictionEffect = static_cast<uint32_t>(_drugAddictionPerkCombo->currentIndex());
    if (_drugAddictionDelayEdit) _drugData.addictionOnset = static_cast<uint32_t>(_drugAddictionDelayEdit->value());
...
bool ProDrugWidget::canHandle(const std::shared_ptr<Pro>& pro) const {
    return pro && pro->type() == Pro::OBJECT_TYPE::ITEM && 
           pro->itemType() == Pro::ITEM_TYPE::DRUG;
}
