...
void ProWeaponWidget::setupUI() {
    ...
    _weaponAnimationCombo = createComboBox({
        "None", "Knife", "Club", "Hammer", "Spear", "Pistol", 
    ...
    _weaponDamageTypeCombo = createComboBox({
    ...
    ammoGroup->setStyleSheet("QGroupBox { font-weight: bold; }");
    ...
    _weaponAmmoTypeCombo = createComboBox({
        "None", ".223 FMJ", "5mm JHP", "5mm AP", "10mm JHP", "10mm AP", 
...
void ProWeaponWidget::loadFromPro(const std::shared_ptr<Pro>& pro) {
    ...
    // Update UI controls
    if (_weaponAnimationCombo) _weaponAnimationCombo->setCurrentIndex(static_cast<int>(_weaponData.animationCode));
    if (_weaponDamageMinEdit) _weaponDamageMinEdit->setValue(static_cast<int>(_weaponData.damageMin));
    if (_weaponDamageMaxEdit) _weaponDamageMaxEdit->setValue(static_cast<int>(_weaponData.damageMax));
    if (_weaponDamageTypeCombo) _weaponDamageTypeCombo->setCurrentIndex(static_cast<int>(_weaponData.damageType));
    if (_weaponRangePrimaryEdit) _weaponRangePrimaryEdit->setValue(static_cast<int>(_weaponData.rangePrimary));
    if (_weaponRangeSecondaryEdit) _weaponRangeSecondaryEdit->setValue(static_cast<int>(_weaponData.rangeSecondary));
    if (_weaponProjectilePIDEdit) _weaponProjectilePIDEdit->setValue(static_cast<int>(_weaponData.projectilePID));
    if (_weaponMinStrengthEdit) _weaponMinStrengthEdit->setValue(static_cast<int>(_weaponData.minimumStrength));
    if (_weaponAPPrimaryEdit) _weaponAPPrimaryEdit->setValue(static_cast<int>(_weaponData.actionCostPrimary));
    if (_weaponAPSecondaryEdit) _weaponAPSecondaryEdit->setValue(static_cast<int>(_weaponData.actionCostSecondary));
    if (_weaponCriticalFailEdit) _weaponCriticalFailEdit->setValue(static_cast<int>(_weaponData.criticalFail));
    if (_weaponPerkCombo) _weaponPerkCombo->setCurrentIndex(static_cast<int>(_weaponData.perk));
    if (_weaponBurstRoundsEdit) _weaponBurstRoundsEdit->setValue(static_cast<int>(_weaponData.burstRounds));
    if (_weaponAmmoTypeCombo) _weaponAmmoTypeCombo->setCurrentIndex(static_cast<int>(_weaponData.ammoType));
    if (_weaponAmmoPIDEdit) _weaponAmmoPIDEdit->setValue(static_cast<int>(_weaponData.ammoPID));
    if (_weaponAmmoCapacityEdit) _weaponAmmoCapacityEdit->setValue(static_cast<int>(_weaponData.ammoCapacity));
    if (_weaponSoundIdEdit) _weaponSoundIdEdit->setValue(static_cast<int>(_weaponData.soundId));
...
void ProWeaponWidget::saveToPro(std::shared_ptr<Pro>& pro) {
    ...
    // Update data from UI
    if (_weaponAnimationCombo) _weaponData.animationCode = static_cast<uint32_t>(_weaponAnimationCombo->currentIndex());
    if (_weaponDamageMinEdit) _weaponData.damageMin = static_cast<uint32_t>(_weaponDamageMinEdit->value());
    if (_weaponDamageMaxEdit) _weaponData.damageMax = static_cast<uint32_t>(_weaponDamageMaxEdit->value());
    if (_weaponDamageTypeCombo) _weaponData.damageType = static_cast<uint32_t>(_weaponDamageTypeCombo->currentIndex());
    if (_weaponRangePrimaryEdit) _weaponData.rangePrimary = static_cast<uint32_t>(_weaponRangePrimaryEdit->value());
    if (_weaponRangeSecondaryEdit) _weaponData.rangeSecondary = static_cast<uint32_t>(_weaponRangeSecondaryEdit->value());
    if (_weaponProjectilePIDEdit) _weaponData.projectilePID = static_cast<int32_t>(_weaponProjectilePIDEdit->value());
    if (_weaponMinStrengthEdit) _weaponData.minimumStrength = static_cast<uint32_t>(_weaponMinStrengthEdit->value());
    if (_weaponAPPrimaryEdit) _weaponData.actionCostPrimary = static_cast<uint32_t>(_weaponAPPrimaryEdit->value());
    if (_weaponAPSecondaryEdit) _weaponData.actionCostSecondary = static_cast<uint32_t>(_weaponAPSecondaryEdit->value());
    if (_weaponCriticalFailEdit) _weaponData.criticalFail = static_cast<uint32_t>(_weaponCriticalFailEdit->value());
    if (_weaponPerkCombo) _weaponData.perk = static_cast<uint32_t>(_weaponPerkCombo->currentIndex());
    if (_weaponBurstRoundsEdit) _weaponData.burstRounds = static_cast<uint32_t>(_weaponBurstRoundsEdit->value());
    if (_weaponAmmoTypeCombo) _weaponData.ammoType = static_cast<uint32_t>(_weaponAmmoTypeCombo->currentIndex());
    if (_weaponAmmoPIDEdit) _weaponData.ammoPID = static_cast<int32_t>(_weaponAmmoPIDEdit->value());
    if (_weaponAmmoCapacityEdit) _weaponData.ammoCapacity = static_cast<uint32_t>(_weaponAmmoCapacityEdit->value());
    if (_weaponSoundIdEdit) _weaponData.soundId = static_cast<uint8_t>(_weaponSoundIdEdit->value());
...
bool ProWeaponWidget::canHandle(const std::shared_ptr<Pro>& pro) const {
    return pro && pro->type() == Pro::OBJECT_TYPE::ITEM && 
           pro->itemType() == Pro::ITEM_TYPE::WEAPON;
}
