...
void MainWindow::setupMenuBar() {
    ...
    connect(redockAllAction, &QAction::triggered, [this]() {
        ...
        if (_mapInfoDock->isFloating()) _mapInfoDock->setFloating(false);
        if (_selectionDock->isFloating()) _selectionDock->setFloating(false);
        if (_tilePaletteDock->isFloating()) _tilePaletteDock->setFloating(false);
        if (_objectPaletteDock->isFloating()) _objectPaletteDock->setFloating(false);
        if (_fileBrowserDock->isFloating()) _fileBrowserDock->setFloating(false);
