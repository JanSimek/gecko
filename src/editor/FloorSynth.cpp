...
namespace floorsynth {
    ...
    namespace {
        ...
        struct Synthesis {
            ...
            int bboxLeft, bboxTop, bboxRight, bboxBottom; ///< region bounding box (cols/rows)
            std::mt19937 rng;
            Stats stats;
            ...
            ConstraintSets constraintSets(int32_t west, int32_t north, int32_t east, int32_t south) const {
                ...
                if (east >= 0) {
                    sets.hasEast = true;
                    sets.toEast = predecessors(model.east, static_cast<uint16_t>(east));
                }
                if (south >= 0) {
                    sets.hasSouth = true;
                    sets.toSouth = predecessors(model.south, static_cast<uint16_t>(south));
                }
                return sets;
            }
            ...
            std::vector<uint16_t> marginalCandidates() const {
                ...
                for (const auto& entry : model.marginal)
                    everything.push_back(entry.first);
                return everything;
            }
