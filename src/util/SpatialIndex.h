...
private:
    struct Item {
        ...
        Item(const T& d, sf::FloatRect b) : data(d), bounds(b) {}
    };
...
template<typename T>
SpatialIndex<T>::SpatialIndex(float cellSize) : _cellSize(cellSize) {
    if (_cellSize <= 0) {
...
void SpatialIndex<T>::queryArea(sf::FloatRect area, QueryCallback callback) const {
    ...
    for (const auto& key : cellKeys) {
        ...
        if (cellIt == _cells.end()) continue;
        
        for (ItemId id : cellIt->second) {
            if (visitedItems.find(id) != visitedItems.end()) continue;
            visitedItems.insert(id);
            
            auto itemIt = _items.find(id);
            if (itemIt == _items.end()) continue;
